...
class eventdates {
	...
	function eventdates() {
		...
		add_action('manage_eventdates_posts_columns',array($this,'columns'),10,2);
		add_action('manage_eventdates_posts_custom_column',array($this,'column_data'),11,2);
	...
	function create_post_type() {
		$labels = array(
			...
			'menu_name'          => 'Event Dates',
			'name_admin_bar'     => 'Event Dates',
			'add_new'            => 'Add New',
			'add_new_item'       => 'Add New Event Date',
			'new_item'           => 'New Event Date',
			'edit_item'          => 'Edit Event Date',
			'view_item'          => 'View Event Date',
			'all_items'          => 'All Event Dates',
			'search_items'       => 'Search Event Dates',
			'parent_item_colon'  => 'Parent Event Dates',
			'not_found'          => 'No Event Dates Found',
			'not_found_in_trash' => 'No Event Dates Found in Trash'
		);
		...
		register_post_type( 'eventdates', $args );
	}
	...
	function create_taxonomies() {
		...
		register_taxonomy('eventdates_type',array('eventdates'),$args);
		...
		register_taxonomy('eventdates_attribute','eventdates',$args);
	}

	function columns($columns) {
		unset($columns['date']);
		unset($columns['taxonomy-eventdates_attribute']);
	...
	function join($wp_join) {
		global $wpdb;
		if(get_query_var('post_type') == 'eventdates') {
			$wp_join .= " LEFT JOIN (
